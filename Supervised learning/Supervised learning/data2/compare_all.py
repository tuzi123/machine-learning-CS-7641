...
print('\n')
"""
Testing the boosted Decision Tree with 3400 training instances,
...
model.add(Dense(input_dim=learning_in.shape[1]/2, output_dim=learning_in.shape[1]/2, init='uniform'))
model.add(Activation('sigmoid'))
model.add(Dense(input_dim=learning_in.shape[1]/2, output_dim=1, init='uniform'))
model.add(Activation('sigmoid'))
model.compile(loss='mean_absolute_error', optimizer='adam', class_mode='binary',metrics=['accuracy'])
model.fit(X_train, Y_train, nb_epoch=ANN_epochs, validation_data=(X_CV, Y_CV), batch_size=10, verbose=0)

ANN_train_error =1- model.evaluate(X_train, Y_train, verbose=0)[1]
ANN_cv_error = 1-model.evaluate(X_CV, Y_CV, verbose=0)[1]
ANN_test_error = 1-model.evaluate(X_test, Y_test, verbose=0)[1]
