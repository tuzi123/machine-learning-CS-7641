...
X_discard, X_train_step, Y_discard, Y_train_step = cross_validation.train_test_split(X_train, Y_train, test_size=1840/X_train.shape[0], random_state=0)
...
plt.legend(loc="best")
fig.savefig('Plots/Boosted Decision Tree Number of Estimators .png')
plt.close()

Best_n_est = cv_score_n_est.index(min(cv_score_n_est))+1
print('best estimators: ',Best_n_est)
...
print('bset learning rate: ',best_LR)
print("time to run:", stop-start)
fig = plt.figure()
...
plt.legend(loc="best")
fig.savefig('Plots/Boosted Decision Tree Learning Rate.png')
plt.close()
...
plt.legend(loc="best")
fig.savefig('Plots/Boosted Decision Tree Tree Depth.png')
plt.close()

best_depth = cv_score.index(min(cv_score))+1
# print(min(cv_score))
print('bset depth: ',best_depth)
...
plt.legend(loc="best")
fig.savefig('Plots/Boosted Decision Tree Error VS Training Size.png')
plt.close()