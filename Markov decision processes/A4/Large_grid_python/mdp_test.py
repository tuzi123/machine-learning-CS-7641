...
class part1():
    def __init__(self):
        ...
        self.rewards = [-1, -1, +100, 0, -1000, +100]
    ...
    def __get_stochastic_action(self, a, success_pct=0.8, directional=True):
        # 0 = up
        # 1 = down
        # 2 = left
        # 3 = right
    ...
    def __convert_grid_to_mdp(self, grid, action_success_pct=1.0, directional=True):
        ...
        for i in range(len(self.actions)):
            for s in range(num_states):
                ...
                t.shape = num_states
                T[i, s] = t
        return T, R, start, goals,r_temp

        
    def run_value_iteration_and_plot(self, grid, k, d, discount, epsilon=0.001,max_iter=10000,plot=True):
        ## policy iteration
        T, R, start, goals,r_temp = self.__convert_grid_to_mdp(grid, k, d)
        vi = mdptoolbox.mdp.ValueIteration(T, R, discount, epsilon=epsilon, max_iter=max_iter)
        
        with open('./output/valueiter.txt', 'a') as text_file:            
            t0 = time()
            vi.run()
            final_time = time() - t0
            if plot: text_file.write('ValueIteration: %0.3f seconds. Iters: %i\n' % (final_time, vi.iter))
        ...
        return final_time, mean_v,vi.iter,vi.policy
        
    def run_policy_iteration_and_plot(self, grid, k, d, discount, epsilon=0.001,max_iter=10000, plot = True):
        ## policy iteration
        T, R, start, goals,r_temp = self.__convert_grid_to_mdp(grid, k, d)
        #pi = mdptoolbox.mdp.PolicyIteration(T, R, discount, max_iter=1000000)
        pi = mdptoolbox.mdp.PolicyIterationModified(T, R, discount, epsilon=epsilon, max_iter=max_iter)
        with open('./output/policyiter.txt', 'a') as text_file:            
            t0 = time()
            pi.run()
            final_time = time() - t0
            if plot: text_file.write('PolicyIteration: %0.3f seconds. Iters: %i\n' % (final_time, pi.iter))
    ...
    def run_and_plot_qlearner(self, grid, d, k, alpha, gamma, rar, rard=0.99, n_restarts=1000, n_iter=100000):
        ...
        fn = './output/' + str(grid.shape[0]) + 'x' + str(grid.shape[1]) + 'Qlearner rewards_iterations_' + str(q.alpha) + '_' + str(q.gamma) + '_' + str(q.orig_rar) + '_' + str(q.radr) + '_' + str(k) + '_' + d_str + '.png'
    ...
    def run(self):
        ...
        for grid_file in ['./input/grid1.csv', './input/grid2.csv']:
            ...
            self.run_value_iteration_and_plot(grid, k=0.8, d=True, discount=0.95, epsilon=0.00001)
            self.run_policy_iteration_and_plot(grid, k=0.8, d=True, discount=0.95, epsilon=0.00001)

            self.run_value_iteration_and_plot(grid, k=1.0, d=True, discount=0.95, epsilon=0.00001)
            self.run_value_iteration_and_plot(grid, k=0.8, d=True, discount=0.95, epsilon=0.00001)
            self.run_value_iteration_and_plot(grid, k=0.7, d=True, discount=0.95, epsilon=0.00001)
        ...
        grid = pd.read_csv(grid_file, header=None).values
        self.run_and_plot_qlearner(grid, d=True, k=0.8, alpha=0.2, gamma=0.8, rar=0.99, rard=0.99, n_restarts=100, n_iter=1000000)
        
        
        grid_file = './input/grid2.csv'
        grid = pd.read_csv(grid_file, header=None).values
        self.run_and_plot_qlearner(grid, d=True, k=0.8, alpha=0.2, gamma=0.8, rar=0.99, rard=0.99, n_restarts=300, n_iter=1000000)
        self.run_and_plot_qlearner(grid, d=True, k=0.8, alpha=0.2, gamma=0.8, rar=0.99, rard=0.999999, n_restarts=300, n_iter=1000000)
