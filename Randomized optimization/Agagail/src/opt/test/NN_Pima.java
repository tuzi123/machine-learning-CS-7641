 ...
 * @version 1.0
 */
public class NN_Pima {

    private static final String BASE_OUTPUT_DIR_PATH = "outputs/tt_neural_net/Pima/Three/";
    ...
    private static int INPUT_LAYERS = 8, HIDDEN_LAYERS = 2, OUTPUT_LAYER = 1, TRAINING_ITERATIONS = 50;
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < oa.length; i++) {
            ...
            //System.out.println(i+ "@ "+optimalInstance.getData());
    ...
    private static Instance[] initializeInstancesTrain() {
        final int NO_INSTANCES = 768;
        double[][][] attributes = new double[NO_INSTANCES][][];
        final int ATT_LENGTH = INPUT_LAYERS;
        try {
            BufferedReader br = new BufferedReader(new FileReader(new File("pima-indians-diabetes.data")));
            for(int i = 0; i < attributes.length; i++) {
                Scanner scan = new Scanner(br.readLine());
                scan.useDelimiter(",");
                attributes[i] = new double[2][];
                attributes[i][0] = new double[ATT_LENGTH]; // 8 attributes
