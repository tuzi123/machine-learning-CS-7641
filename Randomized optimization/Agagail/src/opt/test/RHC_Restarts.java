 ...
 * @version 1.0
 */
public class RHC_Restarts {

    private static final String BASE_OUTPUT_DIR_PATH = "outputs/tt_neural_net/spam/RHC/";
    ...
    private static int INPUT_LAYERS = 57, HIDDEN_LAYERS = 2, OUTPUT_LAYER = 1, TRAINING_ITERATIONS = 800;
    private static BackPropagationNetworkFactory factory = new BackPropagationNetworkFactory();
    
    private static ErrorMeasure measure = new SumOfSquaresError();
    private static int restarts= 3;
    ...
    public static void main(String[] args) {
        if (args.length > 0) 
            restarts = Integer.parseInt(args[0]);
        ...
        for(int i = 0; i < oa.length; i++) {
            ...
            displayResults += oaNames[i] + "," + restarts + "," +  VERSION_NO +
                                ","  + correct + "," + incorrect + "," +
        ...
        try {
            String fileName = getFullFileName("outputs_RHC_" +  VERSION_NO + ".csv");
    ...
    private static void train(OptimizationAlgorithm oa, BackPropagationNetwork network, String oaName) {
        ...
        try {
            String fileName = getFullFileName("errors_" +  VERSION_NO + "_" + restarts + ".csv");
            PrintWriter writer = new PrintWriter(new FileOutputStream(new File(fileName), true));
            writer.println("---");
