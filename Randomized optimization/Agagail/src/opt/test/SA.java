...
public class SA {
    ...
    private static final String BASE_OUTPUT_DIR_PATH = "outputs/tt_neural_net/spam/SA/";
    ...
    private static int INPUT_LAYERS = 57, HIDDEN_LAYERS = 2, OUTPUT_LAYER = 1, TRAINING_ITERATIONS = 750;
    private static BackPropagationNetworkFactory factory = new BackPropagationNetworkFactory();
    
    private static ErrorMeasure measure = new SumOfSquaresError();
    private static int cool_rate= 95;
    ...
    public static void main(String[] args) {
        if (args.length > 0) 
            cool_rate   = Integer.parseInt(args[0]);
        double cooling = cool_rate/100;
        ...
        for(int i = 0; i < oa.length; i++) {
            ...
            displayResults += oaNames[i] + "," + cool_rate + "," +  VERSION_NO +
                                ","  + correct + "," + incorrect + "," +
        ...
        try {
            String fileName = getFullFileName("outputs_SA_" +  VERSION_NO + ".csv");
    ...
    private static void train(OptimizationAlgorithm oa, BackPropagationNetwork network, String oaName) {
        ...
        try {
            String fileName = getFullFileName("errors_" +  VERSION_NO + "_" + cool_rate + ".csv");
            PrintWriter writer = new PrintWriter(new FileOutputStream(new File(fileName), true));
            writer.println("---");
